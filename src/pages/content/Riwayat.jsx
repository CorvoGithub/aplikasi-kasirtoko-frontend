...
const Riwayat = () => {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 space-y-6 md:space-y-8 max-w-7xl mx-auto relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Laporan penjualan yang telah berhasil diproses.
          </p>
        </div>
        ...
        <div className="relative" ref={exportMenuRef}>
          ...
          >
            <Download
              size={18}
              className="text-slate-500 group-hover:text-[#ffad00] transition-colors"
            />
          ...
          {showExportMenu && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 py-1 z-20 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
              <button
                onClick={exportToPDF}
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center gap-4">
        ...
        <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          <div className="relative group">
            ...
            <Calendar
              size={16}
              className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none group-focus-within:text-[#ffad00] transition-colors"
            />
          </div>
                ...
                <tr>
                  <td colSpan="5" className="text-center py-12 text-slate-400">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p>
                        Belum ada transaksi{" "}
                        {filterDate ? "pada tanggal ini" : "tercatat"}.
                      </p>
                    </div>
                  </td>
                </tr>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-slate-500 flex items-center gap-1.5 mt-1">
                          <Clock size={14} /> {getTimePart(trx.created_at)} WIB
                        </span>
                      </div>
                    </td>
      ...
      {/* DETAIL TRANSACTION MODAL (Optimized for Mobile) */}
      {isModalOpen && selectedTransaction && (
        // FIXED: Added h-screen w-screen to fix overlay
        <div className="fixed inset-0 h-screen w-screen bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-100 animate-in fade-in duration-200">
          {/* Added: m-4 to keep off edges, max-h-[90vh] with scroll on body */}
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md m-4 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200 overflow-hidden">
            {/* Header */}
            <div className="bg-[#307fe2] px-6 py-5 text-white text-center relative shrink-0">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                ...
                <div className="space-y-4 mb-6">
                  {selectedTransaction.transaksi_details.map((item, index) => (
                    <div
                      key={index}
                      className="flex justify-between items-start border-b border-dashed border-slate-100 pb-3 last:border-0 last:pb-0"
                    >
                      ...
                      <p className="font-medium text-slate-800 text-sm shrink-0">
                ...
                <div className="space-y-2 pt-4 border-t-2 border-slate-100">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-slate-700">
                      {selectedTransaction.transaksi_details.reduce(
                ...
                <div className="bg-slate-50 rounded-lg p-3 mt-4 text-sm space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-slate-700">
                      {formatRupiah(selectedTransaction.uang_diberikan)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Kembali</span>
                    <span className="font-medium text-emerald-600">
                      {formatRupiah(selectedTransaction.kembalian)}
                    </span>
                  </div>
                </div>
              </div>
