...
import { 
  User, Store, Phone, MapPin, Save, Lock, AlertCircle, Check, 
  ShieldCheck, Camera, HelpCircle, X, ZoomIn
} from 'lucide-react';
...
const ProfilToko = () => {
  ...
  // Loading state specifically for file reading/processing
  ...
  // 1. User selects file -> Read it -> Open Cropper
  const handleFileChange = async (e) => {
    if (e.target.files && e.target.files.length > 0) {
      setIsFileProcessing(true); // START SPINNER
      ...
      try {
          ...
          setIsFileProcessing(false); // STOP SPINNER
  ...
  const executeProfileUpdate = async () => {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            setConfirmModal({ ...confirmModal, isOpen: false, isProcessing: false }); 
  ...
  const initiateSavePassword = (e) => {
    ...
    setConfirmModal({
        isOpen: true,
        title: 'Ubah Password?',
        message: 'Anda akan otomatis logout dan harus login ulang setelah mengubah password.',
  ...
  return (
    <div className="p-6 lg:p-8 space-y-8 max-w-6xl mx-auto relative min-h-screen">
      ...
      {/* ================= CROPPER MODAL ================= */}
      {isCropping && (
        // FIXED: Added h-screen w-screen to fix the gap issue
        <div className="fixed inset-0 h-screen w-screen z-150 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col h-[500px]">
                ...
                <div className="flex justify-between items-center p-4 border-b border-slate-100">
                    ...
                    <button onClick={() => setIsCropping(false)} className="p-1 hover:bg-slate-100 rounded-full transition-colors">
                ...
                <div className="p-4 bg-white border-t border-slate-100 space-y-4">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button onClick={showCroppedImage} className="flex-1 py-2.5 bg-[#307fe2] hover:bg-blue-700 text-white rounded-xl font-semibold shadow-lg shadow-blue-200 transition-colors">
                            Terapkan
                        </button>
                    </div>
                </div>
        ...
        <div className="fixed inset-0 h-screen w-screen z-100 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
           <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200">
              <div className="bg-[#307fe2] p-6 text-center text-white relative overflow-hidden">
                 ...
                 <div className="absolute inset-0 opacity-25" style={{ backgroundImage: 'radial-gradient(circle, #ffffff 1px, transparent 1px)', backgroundSize: '12px 12px' }}></div>
              </div>
      ...
      <div className="relative">
        <div className="h-64 rounded-3xl bg-linear-to-br from-[#307fe2] to-[#2563b0] relative overflow-hidden shadow-xl shadow-blue-200/50 flex flex-col items-center justify-center text-white">
            <div className="absolute inset-0 opacity-25" style={{ backgroundImage: 'radial-gradient(circle, #ffffff 1.5px, transparent 1.5px)', backgroundSize: '24px 24px' }}></div>
            <div className="relative z-10 flex flex-col items-center">
                ...
                <div className="relative group mb-4">
                    ...
                    {/* FIXED: Camera button now shows spinner while processing */}
                    <button 
                ...
                <h1 className="text-3xl font-bold tracking-tight mb-1">{user.store_name || "Nama Toko"}</h1>
        ...
        <div className="lg:col-span-2 flex flex-col h-full">
            <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm h-full flex flex-col">
                <div className="flex items-center gap-4 mb-6 pb-2">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-slate-800">Detail Toko</h2>
                        <p className="text-xs text-slate-500">Informasi dasar toko Anda</p>
                    </div>
                </div>
                <form onSubmit={initiateSaveProfile} className="space-y-6 flex-1 flex flex-col">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div className="relative group">
                                ...
                                <input type="text" required className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-[#307fe2] focus:ring-4 focus:ring-[#307fe2]/10 transition-all text-sm font-medium"
                        ...
                        <div>
                            ...
                            <div className="relative group">
                                ...
                                <input type="text" required className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-[#307fe2] focus:ring-4 focus:ring-[#307fe2]/10 transition-all text-sm font-medium"
                                    value={user.store_name} onChange={e => setUser({...user, store_name: e.target.value})} />
                            </div>
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">Kontak / WhatsApp <span className="text-slate-400 font-normal">(Opsional)</span></label>
                        <div className="relative group">
                            ...
                            <input type="text" className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-[#307fe2] focus:ring-4 focus:ring-[#307fe2]/10 transition-all text-sm font-medium"
                                placeholder="0812..." value={user.phone} onChange={e => setUser({...user, phone: e.target.value})} />
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">Alamat Lengkap <span className="text-slate-400 font-normal">(Opsional)</span></label>
                        <div className="relative group">
                            <MapPin className="absolute left-3 top-4 text-slate-400 group-focus-within:text-[#307fe2] transition-colors" size={18} />
                            <textarea className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-[#307fe2] focus:ring-4 focus:ring-[#307fe2]/10 transition-all text-sm font-medium min-h-[100px]"
                                placeholder="Jl. Raya No. 123, Kecamatan..." value={user.address} onChange={e => setUser({...user, address: e.target.value})} />
                        </div>
                        <p className="text-xs text-slate-400 mt-2 text-right">Foto profil/logo, alamat, dan kontak ini akan dicetak pada struk belanja.</p>
                    </div>
                    <div className="mt-auto pt-4 flex justify-end">
                        <button type="submit" className="px-8 py-3 bg-[#307fe2] hover:bg-blue-700 text-white font-semibold rounded-xl shadow-lg shadow-blue-200 transition-all flex items-center gap-2 active:scale-95">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm h-full flex flex-col">
                <div className="p-6 pb-0">
                    ...
                    <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 flex items-start gap-3">
                        <ShieldCheck className="text-orange-500 mt-0.5 shrink-0" size={20} />
                        <p className="text-xs text-orange-700 leading-relaxed">
                            Gunakan password yang kuat untuk menjaga keamanan data transaksi dan stok toko Anda.
                        </p>
                    </div>
                </div>
                <form onSubmit={initiateSavePassword} className="p-6 space-y-5 flex-1 flex flex-col">
                    <div>
                        ...
                        <div className="relative group">
                            ...
                            <input type="password" required className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 transition-all text-sm font-medium"
                    ...
                    <div>
                        ...
                        <div className="relative group">
                            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-orange-500 transition-colors" size={18} />
                            <input type="password" required className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 transition-all text-sm font-medium"
                                value={passData.password_confirmation} onChange={e => setPassData({...passData, password_confirmation: e.target.value})} />
                        </div>
                    </div>
                    <div className="mt-auto pt-4">
                        <button type="submit" className="w-full bg-slate-800 hover:bg-slate-900 text-white font-semibold py-3 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 active:scale-95">
                            Ubah Password
                        </button>
                    </div>
                </form>
