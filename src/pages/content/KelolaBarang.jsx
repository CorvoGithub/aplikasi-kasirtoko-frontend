...
const KelolaBarang = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        await axios.post(
          `http://localhost:8000/api/products/${currentId}`,
          data,
          config
        );
  ...
  const executeDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `http://localhost:8000/api/products/${deleteModal.id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchProducts();
  ...
  const openEditModal = (product) => {
    ...
    setPreviewUrl(
      product.foto ? `http://localhost:8000/storage/${product.foto}` : null
    );
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 space-y-6 md:space-y-8 max-w-7xl mx-auto relative">
      {/* Toast Notification */}
      {toast.show && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 bg-slate-50 text-center">
              <p className="text-slate-600 text-sm mb-6">
                ...
                <strong>"{deleteModal.name}"</strong>? Data yang dihapus tidak
                dapat dikembalikan.
              </p>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Tambahkan, edit, atau hapus produk toko Anda.
          </p>
        </div>
        <button
          onClick={openAddModal}
          className="flex items-center gap-2 bg-[#307fe2] hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-medium shadow-lg shadow-blue-200 transition-all"
        >
      ...
      {/* Search Bar */}
      <div className="bg-white p-2 rounded-2xl border border-slate-200 shadow-sm flex items-center w-full max-w-md group focus-within:border-[#ffad00] focus-within:ring-4 focus-within:ring-[#ffad00]/10 transition-all">
              ...
              {loading ? (
                ...
                currentItems.map((product) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right text-[#307fe2] font-medium">
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex gap-3">
              ...
              <button
                ...
                form="productForm"
                disabled={submitLoading}
                className="flex-1 px-4 py-2.5 bg-[#307fe2] text-white font-semibold rounded-xl hover:bg-blue-700 shadow-md shadow-blue-200 transition-all disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {submitLoading ? (
                  <>
                    <div className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full"></div>
