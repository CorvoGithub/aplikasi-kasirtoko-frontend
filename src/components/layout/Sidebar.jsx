  ...
  return (
    <div className={sidebarClasses}>
      ...
      <nav className="flex-1 px-4 py-8 overflow-y-auto relative custom-scrollbar">
        ...
        {/* Sliding Indicator */}
        <div
          className={`absolute left-4 right-4 bg-blue-50 rounded-xl z-0 transition-all duration-300 ease-[cubic-bezier(0.25,0.1,0.25,1.0)] ${
            activeIndex === -1 ? "opacity-0 scale-95" : "opacity-100 scale-100"
          }`}
          style={{
            height: `${ITEM_HEIGHT}px`,
            transform: `translateY(${
              activeIndex * (ITEM_HEIGHT + ITEM_GAP)
            }px)`,
          }}
        >
                ...
                <>
                  <span
                    className={`transition-colors ${
                      isActive ? "text-[#307fe2]" : "text-slate-400"
                    }`}
                  >
                    {item.icon}
                  </span>
